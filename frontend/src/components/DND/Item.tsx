        ...
        <div className="grid grid-cols-12 w-full border border-gray-300 rounded-lg p-1.5 justify-between">
            <div className={`${isDetails ? "col-span-full" : "col-span-8"}`}>
                <div className="flex items-center">
                    ...
                    <button onClick={() => setIsDetails((prev) => !prev)} className="text-center text-xs text-gray-500 ml-2 mt-1">{isDetails ? "Hide details" : "View details"}{'>'}</button>
                </div>
                <div className={`w-full text-sm text-opacity-90 text-black relative group pr-1 z-[999] ${isDetails ? "min-h-max" : "truncate"}`}>{task.description}
                </div>
            </div>
